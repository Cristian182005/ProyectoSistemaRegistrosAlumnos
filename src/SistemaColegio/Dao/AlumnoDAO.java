...
public class AlumnoDAO implements Dao<Alumno> {
    ...
    public boolean registrar(Alumno alumno) {
        String sql = "INSERT INTO alumnos (idAlumno, apoderado, grado, seccion, dni, nombres, apellidoP, apellidoM, fechaNacimiento, sexo, direccion, telefono, email) " +
                     "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement stmt = conexion.prepareStatement(sql)) {
            ...
            stmt.setString(8, alumno.getApellidM());
    ...
    public boolean actualizar(Alumno alumno) {
        String sql = "UPDATE alumnos SET apoderado = ?, grado = ?, seccion = ?, nombres = ?, apellidoP = ?, apellidoM = ?, fechaNacimiento = ?, sexo = ?, direccion = ?, telefono = ?, email = ?, dni = ? " +
                     "WHERE idAlumno = ?";
        try (PreparedStatement stmt = conexion.prepareStatement(sql)) {
            ...
            stmt.setString(6, alumno.getApellidM());

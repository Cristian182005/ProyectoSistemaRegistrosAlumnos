...
public class HorarioDAO implements Dao<Horario> {
    ...
    public boolean registrar(Horario horario) {
        String sql = "INSERT INTO horarios (idHorario, diaSemana, horaEntrada, horaSalida, idAula) VALUES (?, ?, ?, ?, ?)";
    ...
    public Horario consultarPorId(int idHorario) {
        ...
        try (PreparedStatement stmt = conexion.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                Aula aula = new Aula();
                aula.setIdAula(rs.getString("idAula")); // Solo id, puedes cargar más luego si quieres
    ...
    public boolean actualizar(Horario horario) {
        String sql = "UPDATE horarios SET diaSemana = ?, horaEntrada = ?, horaSalida = ?, idAula = ? WHERE idHorario = ?";
        try (PreparedStatement stmt = conexion.prepareStatement(sql)) {
    ...
    public List<Horario> listarTodos() {
        ...
        try (Statement stmt = conexion.createStatement();
            ...
            while (rs.next()) {
                Aula aula = new Aula();
                aula.setIdAula(rs.getString("idAula"));
